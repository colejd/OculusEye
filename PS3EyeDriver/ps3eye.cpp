...
#define UVC_STREAM_FID	(1 << 0)
    ...
    uint8_t* PS3EYECam::getLastFramePointerVolatile()
    {
        last_qued_frame_time = urb->last_frame_time;
        uint8_t* frame = (urb->frame_buffer + urb->frame_complete_ind * urb->frame_size);
        //printf("%lu\n", sizeof(frame) / sizeof(frame[0])); //Print length of array?
        return frame;
    }
    
    const unsigned char* PS3EYECam::getLastFramePointerChar()
    {
        last_qued_frame_time = urb->last_frame_time;
        const unsigned char* frame = reinterpret_cast<unsigned char*>(const_cast<uint8_t*>(urb->frame_buffer + urb->frame_complete_ind * urb->frame_size));
        return frame;
    }
