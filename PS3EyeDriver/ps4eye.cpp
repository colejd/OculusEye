...
#define UVC_STREAM_FID	(1 << 0)
    ...
    USBMgr::~USBMgr()
    {
        printf("USBMgr destructor called\n");
    ...
    public:
        ...
        void frame_add(enum gspca_packet_type packet_type, const uint8_t *data, int len)
        {
            ...
            if (len > 0)
            {
                ...
                {
                    printf("discard packet per size %d\n",frame_data_len);
    ...
    {
        printf("PS4EYECAM release called\n");
    ...
    {
        int i;
        printf("BEGIN MULTIWRITE USB brequest a7\n");
    ...
    void PS4EYECam::multi_register_write(const uint16_t data[][2], uint8_t len, uint8_t subaddr)
    {
        ...
        int i;
        printf("BEGIN MULTIWRITE USB brequest a4\n");
        /* int size= ARRAY_SIZE(data);
         printf("size es %d\n",size);*/
        uint8_t *buffer_out=new unsigned char[len*4+16]();
        switch (subaddr) {
    ...
    void PS4EYECam::dump_registers(uint16_t start,uint16_t finish,uint8_t subaddr)
    {
        ...
        uint8_t val=0;
        printf("READ register BEGIN from 0x%04x - 0x%04x\n ",start,finish);
        while(start<=finish)
        {
            ...
            {
                printf("READ register 0x%04x: READ ERROR ",start);
            }
            //   sleep(1);
            start++;
        }
        printf("READ FROM SUBADDR 0x%02x END \n",subaddr);
    }
    /*
    ...
    void PS4EYECam::multi_register_read(const uint16_t (*data),uint8_t len, uint8_t subaddr)
    {
        ...
        for(i=0;i<len;i++)
        {
            ...
            {
                printf("READ register 0x%04x: READ ERROR ",data[i]);
            }
            // sleep(1);
        }
    }
    ...
    {
        //printf("reg: 0x%02x, val: 0x%02x", reg, val);
    ...
    uint8_t PS4EYECam::register_read(uint16_t reg,uint8_t subaddr)
    {
        ...
        if(aux!=reg)
        {
            printf("phase 2 read failed register read  0x%04x not 0x%04x \n",aux,reg);
    ...
    {
        printf("stop_sensor_streaming called\n");
    ...
    void PS4EYECam::dump_sensor_generalconf(uint8_t n)
    {
        ...
        {
            case 1:
                printf("Dumping Sensor 1 general conf registers\n");
                break;
            case 2:
                printf("Dumping Sensor 2 general conf registers\n");
                break;
            default:
                printf("%d is not a valid sensor number\n",n);
                return;
                break;
        }
    ...
    void PS4EYECam::set_sensor_video_mode(uint8_t mode, uint8_t n)
    {
        ...
        switch(mode)
        {
            ...
            case 28:
                ...
                break;
            default:
                printf("%d is not a valid mode number\n",mode);
                break;
        }
    ...
    bool PS4EYECam::read_sensor_id(uint8_t n)
    {
        ...
        printf("read sensor_id from sensor %d\n",n);
        ...
        printf("sensor %d id 0x%04x \n",n,sensor_id);
        if(sensor_id!=0x9713)
        {
            printf("Sensor %d is not ready, exit\n",n);
           // stop_sensors_streaming();
           // reset_sensors();
            //put led off
            //set_led_off();
    ...
    bool PS4EYECam::init(uint32_t width, uint32_t height, uint8_t desiredFrameRate)
    {
        ...
        if(!check_sensors())
        {
            printf("Sanity check from sensors failed\n");
            //set_led_off();
            //reset_sensors();
            return false;
        }
    /*  TODO play with video modes perhaps we will need to go to uvc path without ov580 information.
        printf("Show video mode registers from sensor 1\n");
        dump_sensor_video_mode(1);
        printf("Show video mode registers from sensor 2\n");
        dump_sensor_video_mode(2);
...
/*
        if(!init_registers())
        {
            printf("Initialization registers failed\n");
            return false;
        }
*/
        return true;
    ...
    void PS4EYECam::stop()
    {
        printf("stop is called is_streaming is: %d\n",is_streaming);
        if(!is_streaming) return;
        //set led off
    ...
    {
        printf("PS4EYECAM shutdown called wait...\n");
        stop();
        sleep(2);
        urb->num_transfers=0;
       // release();
    ...
    void PS4EYECam::check_ff71()
    {
        ...
        switch(urb->ff71status)
        {
            case 0:
                    ...
                    if(val==0x0)
                    {
                        printf("i=%d Write register 0xff70 to f1\n",urb->ff71status);
                     //   usleep(1);
            ...
            case 1:
                    ...
                    printf("i=%d Write register 0xff70 to 01\n",urb->ff71status);
  ...
  /*  const uint8_t* PS4EYECam::getLastVideoFramePointer()
    ...
    void PS4EYECam::dump_device()
    {
        ...
        {
            printf("error getting config descriptors\n");
            return;
        }
        dump_config(config);
   	...
   	int PS4EYECam::uvc_set_video_mode(uint8_t mode,uint8_t fps)
	{
		...
		switch(mode)
		{
			case 0://3448x808
				...
				{
					printf("Unsupported fps=%d take default 60 fps\n",fps);
					frame_rate=60;
				}
				else
				{
		...
		printf("Frame resolutiion: %dx%d framerate: %d \n",frame_width,frame_height,frame_rate);
		ctrl.bmHint=1;
		ctrl.bFormatIndex=1;
        ...
        if (err == len)
        {
            printf("control transfer for video set return %d\n",err);
            return 0;
        }
        else
        {
            printf("control transfer for video set error return %d\n",err);
            return err;
        }
	}
    int PS4EYECam::uvc_show_video_mode()
    {
        ...
        if (err == len)
        {
            printf("control transfer for video get return %d\n",err);
        ...
        {
            printf("control transfer for video ger error return %d\n",err);
            return err;
        }
    ...
    bool PS4EYECam::open_usb()
    {
        ...
        if(res != 0) {
            printf("set alt setting to interface error: %d\n", res);
            return false;
        }
        printf("set alt to 1 in interface 1\n");
        uvc_show_video_mode();
    ...
    void  LIBUSB_CALL PS4EYECam::ct_done(struct libusb_transfer * transfer) {
        ...
        if (cam->control_wLength != transfer->actual_length) {
            printf("phase 2 read failed received %d bytes instead of %d\n",transfer->actual_length ,cam->control_wLength);
            cam->controlTransferError = true;
        }
